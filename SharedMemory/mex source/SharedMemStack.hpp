...
class shared_mem_stack {
	...
	public:
		...
		int removeSegmentByIndex(int index, bool force);				/* removes the segement with the given index.  returns 0 if successful */
...
/*  Function to remove a segment from the buffer */
/*	Returns -1 if the segment is not found, or the number of variables still attached to it        */
/*  If force=true, the segment is removed even if refCount>0 */
int shared_mem_stack::removeSegmentByIndex(int index, bool force)
{
	int i, ret = -1;						/* counter */
	...
	ret = (int)pMemSegments[index].refCount;

	/*  don't destroy the segment if variables are attached and force=false */
	if (pMemSegments[index].refCount != 0 && !force)
		return ret;
