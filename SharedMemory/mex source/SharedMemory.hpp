...
const size_t  align_size = 16;   /*the pointer alignment size, so if pdata is a valid pointer then &pdata[i*align_size] will also be.  Ensure this is >= 4*/
...
/* shared memory.                                                            */
size_t deepcopy       (header_t *hdr, data_t *dat, char *shared_mem, 
					   header_t*  par_hdr, mxArray *mxInput);
